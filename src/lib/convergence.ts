...
const CORRIDORS: Record<string, ConvergenceCorridor> = {
  "mae-sot-myawaddy": {
    id: "mae-sot-myawaddy",
    label: "Mae Sot / Myawaddy Corridor",
    center: [98.56, 16.71],
    radiusKm: 120,
    aliases: [
      "Mae Sot",
...
const ALERT_TEMPLATES: AlertTemplate[] = [
  {
    id: "cross-border-disruption",
    title: "Cross-border disruption risk",
    families: ["incident", "news", "market"],
    emptySummary:
      "No corroborated disruption signal is active beyond baseline monitoring.",
  },
  {
    id: "market-logistics-stress",
    title: "Market and logistics stress",
    families: ["market", "news", "incident"],
    emptySummary:
      "Trade and FX conditions are not yet confirming a corridor stress spike.",
  },
  {
    id: "humanitarian-weather-pressure",
    title: "Humanitarian and weather pressure",
    families: ["weather", "movement", "thermal", "news"],
    emptySummary:
      "Weather, displacement, and thermal indicators are currently acting as background context.",
  },
];

const memoryCache = new Map<string, CachedConvergenceSnapshot>();

function getCorridor(id = "mae-sot-myawaddy") {
...
function buildReasonPrefix(
  ...
  switch (family) {
    case "incident":
      return `${context}; field signal intersects the corridor.`;
    case "news":
      return `${context}; reporting corroborates corridor relevance.`;
    case "market":
      return `${context}; trade or FX stress is corridor-relevant.`;
    case "weather":
      return `${context}; rainfall shift can affect access and operations.`;
    case "thermal":
      return `${context}; thermal activity overlaps the corridor radius.`;
    case "movement":
      return `${context}; cross-border movement pressure remains elevated.`;
    default:
      return context;
  }
}
...
function buildNewsEvidence(
  ...
  return dedupeByTitle(
    items.filter((item) => {
      ...
      return item.packageId === "western-border" || corridorMatchesText(corridor, text);
    }),
  )
...
function buildMarketEvidence(
  ...
  return indicators
    .filter((indicator) =>
      /thb\/mmk|border trade|trade|logistics|diesel|fuel|rice|forex|fx|customs|tak|mae sot|myawaddy/i.test(
    ...
    .map<ConvergenceEvidence>((indicator, index) => {
      ...
      const magnitude =
        change >= 1.5 || /thb\/mmk|border trade/i.test(indicator.label)
          ? "alert"
          : change >= 0.5
            ? "watch"
            : "stable";
      ...
      return {
        ...
        family: "market",
        title: `${indicator.label} corridor stress`,
...
function buildWeatherEvidence(
  ...
  return rainfall
    ...
    .map<ConvergenceEvidence>((point, index) => {
      ...
      const magnitude = value >= 10 ? "alert" : value >= 6 ? "watch" : "stable";
      const observedAt = new Date().toISOString();
  ...
  return movements
    .filter(
      (movement) =>
        ...
        /myanmar/i.test(movement.label),
    )
    .map<ConvergenceEvidence>((movement, index) => {
      const magnitude = movement.count >= 50000 ? "alert" : movement.count >= 10000 ? "watch" : "stable";
      const observedAt = new Date().toISOString();
...
function buildAlertSummary(alert: AlertTemplate, evidence: ConvergenceEvidence[]) {
  ...
  return `${lead.title} is the lead ${lead.family} signal, with ${evidence.length} corroborating items inside the ${ALERT_WINDOW_HOURS}-hour watch window.`;
}
...
function buildDataGaps(
  ...
  if (missingFamilies.length > 0) {
    gaps.push(`No direct ${missingFamilies.join(", ")} evidence in corridor scope.`);
  }
...
function buildTopLevelSummary(
  ...
  if (posture === "priority") {
    return `${corridor.label} is on priority posture. ${leadFamilies} are converging, led by ${leadAlert?.title.toLowerCase() ?? "corridor disruption risk"}.`;
  }

  if (posture === "watch") {
    return `${corridor.label} is on watch posture with a convergence score of ${score}. ${leadFamilies} are starting to reinforce each other.`;
  }

  return `${corridor.label} remains in monitor posture. The corridor has background pressure, but corroboration is not yet strong enough for escalation.`;
}
...
async function buildCorridorConvergence(
  ...
  const dataGaps = [
    ...(["incident", "news", "market", "weather", "thermal", "movement"] as const)
      ...
      .map((family) => `No corridor-matched ${family} evidence is currently active.`),
...
export function getConvergenceCorridor(id = "mae-sot-myawaddy") {
  return getCorridor(id);
}

export async function loadCorridorConvergence(
  corridorId = "mae-sot-myawaddy",
): Promise<CorridorConvergenceResponse> {
