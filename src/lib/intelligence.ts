...
const KEYWORD_GROUPS = [
  ...
  },
  {
    tag: "spillover",
    weight: 16,
    terms: ["spillover", "karen", "refugee", "displacement", "shelling"],
  },
  {
    tag: "deep-south",
    weight: 18,
  ...
  },
  {
    tag: "fx",
    weight: 12,
    terms: ["thb", "baht", "mmk", "kyat", "foreign exchange", "currency"],
  },
  {
    tag: "weather",
    weight: 12,
    terms: ["rain", "flood", "storm", "weather", "precipitation", "landslide"],
  },
  {
    tag: "humanitarian",
    weight: 14,
    terms: ["humanitarian", "refugee", "displacement", "shelter", "aid", "camp"],
  },
  {
    tag: "thermal",
    weight: 10,
    terms: ["fire", "thermal", "hotspot", "burn", "wildfire"],
  },
  {
    tag: "violence",
    weight: 18,
...
const PACKAGE_DEFINITIONS: PackageDefinition[] = [
  {
    id: "western-border",
    title: "Western Border / Myanmar Spillover",
    description:
      "Cross-border instability, logistics pressure, and movement signals around Mae Sot, Tak, and adjacent corridors.",
    sourceIds: ["bangkok-post", "cna", "bbc-world", "city-reporter-news"],
    queries: [
      "Thailand Myanmar border Mae Sot OR Tak OR Myawaddy",
      "Myanmar spillover Thailand border refugees OR shelling",
    ],
    focusTags: ["border", "spillover", "violence", "humanitarian", "trade"],
    anchorTags: ["border", "spillover"],
    matcher: (item) =>
      /mae sot|tak|mae hong son|myawaddy|myanmar|karen|border/i.test(
        `${item.title} ${item.summary}`,
      ),
  },
  {
    id: "deep-south",
    title: "Deep South Security",
    description:
      "Security, insurgency, checkpoint posture, and local violence signals in Narathiwat, Pattani, and Yala.",
    sourceIds: ["bangkok-post", "cna", "bbc-world"],
    queries: [
      "Narathiwat Pattani Yala attack OR checkpoint OR security",
      "Thailand deep south insurgency OR bombing OR raid",
    ],
    focusTags: ["deep-south", "violence", "border"],
    anchorTags: ["deep-south"],
    matcher: (item) =>
      /narathiwat|pattani|yala|deep south|southern thailand|insurgency/i.test(
        `${item.title} ${item.summary}`,
      ),
  },
  {
    id: "trade-fx",
    title: "Trade, Logistics, and FX Stress",
    description:
      "Border trade, currency spread, supply corridors, customs friction, and market spillover into frontier operations.",
    sourceIds: ["bangkok-post", "nikkei-asia", "guardian-world", "cna"],
    queries: [
      "Thailand Myanmar trade corridor baht kyat customs logistics",
      "Thailand border trade fuel rice logistics baht",
    ],
    focusTags: ["trade", "fx", "border", "spillover"],
    anchorTags: ["trade", "fx", "border"],
    matcher: (item) => {
      const text = `${item.title} ${item.summary}`;
      return (
        /thailand|myanmar|mae sot|tak\b|myawaddy|border trade|cross-border|customs|corridor|thai navy|cargo ship/i.test(
          text,
        ) &&
        /trade|logistics|diesel|fuel|rice|thb|baht|mmk|kyat|currency|cargo|market/i.test(
          text,
        )
      );
    },
  },
  {
    ...
    title: "Humanitarian Movement and Weather Pressure",
    description:
      "Rainfall, mobility pressure, displacement, camp stress, and weather-linked operational friction across frontier provinces.",
    sourceIds: ["cna", "bbc-world", "guardian-world", "city-reporter-news"],
    queries: [
      "Thailand Myanmar refugees flood rain displacement border",
      "Thailand border rainfall flooding humanitarian camps",
    ],
    focusTags: ["humanitarian", "weather", "thermal", "spillover"],
    anchorTags: ["humanitarian", "weather", "spillover"],
    matcher: (item) => {
      const text = `${item.title} ${item.summary}`;
      return (
        /thailand|myanmar|mae sot|tak\b|myawaddy|karen|banteay meanchey|cambodia|thai border|thailand-myanmar/i.test(
          text,
        ) &&
        /camp|refugee|displacement|movement|humanitarian|rain|flood|weather|monsoon|hotspot|fire|aid/i.test(
          text,
        )
      );
    },
...
function isPriorityNewsRelevant(item: SeedItem) {
  ...
  if (
    /thailand monitor|bangkok post|thai pbs|karen news|camboja|city reporter|channel newsasia/i.test(
      item.source,
    )
  ) {
    return true;
  ...
  return /thailand|thai\b|myanmar|burma|mae sot|tak\b|myawaddy|karen|narathiwat|pattani|yala|deep south|baht|kyat|thb|mmk|cambodia|banteay meanchey/i.test(
...
export async function loadRefugeeMovements(): Promise<RefugeeMovement[]> {
  try {
    const res = await query<RefugeeMovementRow>(`
      SELECT origin_country, asylum_country, count, ref_year
      FROM population_movements
      ORDER BY ref_year DESC, count DESC
      LIMIT 20
    `);
    ...
    return res.rows.map((row) => ({
      source:
        row.origin_country === "Myanmar"
          ? [98.5, 16.7]
          : row.origin_country === "Cambodia"
            ? [104.2, 12.5]
            : [98, 18],
      target:
        row.asylum_country === "Thailand"
          ? [100.5, 13.7]
          : row.asylum_country === "Laos"
            ? [102.6, 18.0]
            : [100, 14],
      count: row.count,
...
function buildIncidentSeeds(incidents: IncidentFeature[]): SeedItem[] {
  return incidents.slice(0, 20).map((incident, index) => {
    ...
    const keywordSignals = readKeywordSignals(combinedText, ["border", "violence"]);
...
function buildMarketSeeds(indicators: EconomicIndicator[]): SeedItem[] {
  return indicators.slice(0, 8).map((indicator, index) => {
    ...
    const keywordSignals = readKeywordSignals(combinedText, ["trade", "fx"]);
    const magnitude =
    ...
    return {
      ...
      url: "/api/markets",
      tags: Array.from(new Set([...keywordSignals.tags, "trade", "fx"])).slice(0, 4),
      score,
...
function buildRainfallSeeds(rainfall: RainfallPoint[]): SeedItem[] {
  return rainfall.slice(0, 8).map((point, index) => {
    ...
    return {
      ...
      title: `${point.label} rainfall anomaly`,
      summary: `${point.label} is reading a rainfall shift of ${point.value.toFixed(1)}.`,
      source: "Rainfall cache",
      sourceUrl: "/api/rainfall",
      publishedAt: new Date().toISOString(),
      url: "/api/rainfall",
      tags: ["weather", "humanitarian"],
      score,
      severity: Math.abs(point.value) >= 10 ? "watch" : "stable",
      kind: "weather",
    };
  });
}
...
function buildMovementSeeds(movements: RefugeeMovement[]): SeedItem[] {
  return movements.slice(0, 6).map((movement, index) => ({
    ...
    title: "Population movement pressure",
    summary: movement.label,
    source: "Movement cache",
    sourceUrl: "/api/refugees",
    publishedAt: new Date().toISOString(),
    url: "/api/refugees",
    tags: ["humanitarian", "spillover", "border"],
    score: 14 + Math.log10(movement.count + 1) * 8,
    severity: movement.count >= 50000 ? "alert" : "watch",
    kind: "movement",
  }));
}
...
function buildHeuristicPackageCopy(
  ...
  if (!lead) {
    return {
      headline: `${definition.title} remains on monitored watch.`,
  ...
  return {
    ...
    summary: `${items.length} ranked signals are active, with ${lead.source} currently leading the package. Dominant themes: ${dominantTags.join(", ") || "general watch"}.`,
    priorities: [
      ...
      dominantTags.length > 0
        ? `Watch for convergence between ${dominantTags.join(", ")} signals and market stress.`
        : "Watch for convergence between field signals and market stress.",
    ],
  };
}
      ...
      packages: [
        {
          id: "western-border",
          title: "Western Border / Myanmar Spillover",
          headline: "Western border watch remains active around Mae Sot.",
          summary:
            "Fallback intelligence package: use field reports, market cards, and map overlays together until live sources recover.",
          description:
            "Cross-border instability, logistics pressure, and movement signals around Mae Sot, Tak, and adjacent corridors.",
          priorities: [
            "Keep Mae Sot and Tak in the active review loop.",
            "Check FX spread and movement signals against the incident map.",
            "Refresh vegetation, rain, and thermal overlays in the next cycle.",
          ],
          dominantTags: ["border", "spillover", "trade"],
          sourceLabels: ["Fallback intelligence"],
          updatedAt: fallbackNews.generatedAt,
          status: "offline",
          items: buildIncidentSeeds(fallbackIncidents)
            .slice(0, 3)
            .map((item, index) => ({
              ...item,
              id: `western-border-fallback-${index + 1}`,
              packageId: "western-border",
            })),
          stats: {
            total: 3,
            elevated: 2,
        ...
        },
        {
          id: "deep-south",
          title: "Deep South Security",
          headline: "Deep south monitoring remains on watch posture.",
          summary:
            "Fallback package focused on Narathiwat, Pattani, and Yala until live feeds resume.",
          description:
            "Security, insurgency, checkpoint posture, and local violence signals in Narathiwat, Pattani, and Yala.",
          priorities: [
            "Keep Narathiwat, Pattani, and Yala on the watch board.",
            "Cross-check incident reports against package items before escalation.",
            "Refresh the map against active rainfall and movement layers.",
          ],
          dominantTags: ["deep-south", "violence"],
          sourceLabels: ["Fallback intelligence"],
          updatedAt: fallbackNews.generatedAt,
          status: "offline",
          items: buildIncidentSeeds(fallbackIncidents)
            ...
            .map((item, index) => ({
              ...item,
              id: `deep-south-fallback-${index + 1}`,
              packageId: "deep-south",
            })),
          stats: {
            total: 2,
            elevated: 1,
        ...
        },
        {
          id: "trade-fx",
          title: "Trade, Logistics, and FX Stress",
          headline: "THB/MMK remains the lead cross-border market signal.",
          summary:
            "Fallback package using available economics and trade indicators.",
          description:
            "Border trade, currency spread, supply corridors, customs friction, and market spillover into frontier operations.",
          priorities: [
            "Track THB/MMK and corridor trade pressure together.",
            "Use market radar as the lead economic stress readout.",
            "Escalate only when field and market indicators converge.",
          ],
          dominantTags: ["trade", "fx"],
        ...
        },
        {
          id: "humanitarian-weather",
          title: "Humanitarian Movement and Weather Pressure",
          headline: "Weather and movement remain the main humanitarian watch factors.",
          summary:
            "Fallback package based on rainfall, movement, and thermal anomaly signals.",
          description:
            "Rainfall, mobility pressure, displacement, camp stress, and weather-linked operational friction across frontier provinces.",
          priorities: [
            "Refresh rainfall and movement overlays before issuing area updates.",
            "Track thermal anomalies where rainfall and access constraints overlap.",
            "Use humanitarian indicators as context, not in isolation.",
          ],
          dominantTags: ["weather", "humanitarian", "thermal"],
          sourceLabels: ["Fallback environment"],
          updatedAt: fallbackTicker.generatedAt,
          status: "offline",
          items: [
            ...
            ...item,
            id: `humanitarian-weather-fallback-${index + 1}`,
            packageId: "humanitarian-weather",
          })),
          stats: {
            total: 3,
            elevated: 2,
...
export async function buildTickerFromPackages(): Promise<TickerResponse> {
  ...
  const dominantTheme = topPackage?.dominantTags[0] ?? "watch";
