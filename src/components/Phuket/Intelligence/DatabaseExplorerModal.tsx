      ...
      >
          ...
          <div className="flex items-center gap-3">
            {authToken ? (
              <button
                type="button"
                onClick={clearToken}
                className="rounded-full border border-[var(--line-bright)] bg-[var(--bg-surface)] px-3 py-1.5 text-[13px] font-mono uppercase tracking-[0.16em] text-[var(--muted)] transition-colors hover:text-[var(--cool)]"
              >
                Clear token
              </button>
            ) : null}
        ...
        <div className="grid min-h-0 flex-1 gap-4 overflow-hidden px-4 py-4 sm:px-6 sm:py-6 lg:grid-cols-[320px_minmax(0,1fr)]">
          ...
          <main className="min-h-0 overflow-y-auto">
            {error && (
              <div className="mb-4 rounded-2xl border border-[#ef4444] bg-[rgba(127,29,29,0.22)] px-4 py-3 text-[14px] text-[#fecaca]">
                {error}
              </div>
            )}
              ...
              <section className="mb-4 rounded-2xl border border-[var(--line)] bg-[rgba(10,15,26,0.72)] p-4 sm:p-5">
                ...
                <div className="mt-4 flex flex-col gap-3 sm:flex-row">
                  <input
                    ...
                    className="min-w-0 flex-1 rounded-2xl border border-[var(--line)] bg-[var(--bg-surface)] px-4 py-3 text-[15px] text-[var(--ink)] outline-none transition-colors placeholder:text-[var(--dim)] focus:border-[var(--cool)]"
                  />
                  <button
                    type="button"
                    onClick={submitToken}
                    className="inline-flex items-center justify-center rounded-2xl border border-[var(--line-bright)] bg-[var(--bg-surface)] px-4 py-3 text-[14px] font-semibold text-[var(--ink)] transition-colors hover:text-[var(--cool)]"
                  >
                    Unlock
                  </button>
                </div>
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    {PREVIEW_LIMIT_OPTIONS.map((option) => (
                      <button
                        ...
                        className={`rounded-full border px-3 py-1.5 text-[13px] font-bold uppercase tracking-[0.16em] transition-colors ${
                          previewLimit === option
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {selectedTable.columns.map((column) => (
                      <span
                        ...
                        className="rounded-full border border-[var(--line-bright)] bg-[var(--bg-surface)] px-2.5 py-1 text-[13px] font-medium text-[var(--ink)]"
                      >
                        {column}
                      </span>
                    ))}
