...
function AlertCard({ alert }: { alert: ConvergenceAlert }) {
  return (
    <article className="rounded-lg border border-[var(--line)] bg-[var(--bg-raised)] p-3">
      ...
      <div className="mt-3 space-y-2">
        {alert.evidence.slice(0, 2).map((item) => (
          <div
            key={item.id}
            className="rounded-md border border-[var(--line)] bg-[var(--bg)] px-2.5 py-2"
          >
      ...
      {alert.dataGaps.length > 0 ? (
        <div className="mt-3 rounded-md border border-[rgba(245,158,11,0.08)] bg-[rgba(245,158,11,0.06)] px-2.5 py-2">
...
export default function ConvergenceAlerts() {
  ...
  if (!payload) {
    return (
      <div className="rounded-lg border border-[var(--line)] bg-[var(--bg-raised)] p-3 text-[11px] leading-5 text-[var(--muted)]">
        Convergence watchpoints are loading.
      </div>
    );
  }
      ...
      <div className="rounded-lg border border-[var(--line)] bg-[var(--bg-raised)] p-3">
        ...
        <div className="mt-3 grid grid-cols-4 gap-2">
          {[
            ...
            <div
              key={stat.label}
              className="rounded-md border border-[var(--line)] bg-[var(--bg)] px-2 py-2 text-center"
            >
