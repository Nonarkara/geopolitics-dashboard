...
function Pill({ children }: { children: React.ReactNode }) {
  return (
    <span className="rounded-sm border border-[var(--line-bright)] bg-[var(--bg-surface)] px-2.5 py-1 text-[13px] font-medium text-[var(--ink)]">
      {children}
    </span>
  );
}
            ...
            {architectureLayers.map((layer, index) => (
              ...
              >
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <span
                    className={`rounded-sm border px-2.5 py-1 text-[13px] font-bold uppercase tracking-[0.16em] ${toneClasses(layer.tone)}`}
                  >
                    {layer.tone}
                  </span>
                ...
                <div className="mt-3 space-y-2">
                  {layer.bullets.map((bullet) => (
                    <p
                      key={`${layer.id}-${bullet}`}
                      className="rounded-sm border border-[var(--line)] bg-[var(--bg-raised)] px-3 py-2 text-[14px] leading-5 text-[var(--muted)]"
                    >
                      {bullet}
                    </p>
                  ))}
        ...
        <div className="mt-4 space-y-3">
          {architectureFlowSteps.map((step, index) => (
            <article key={step.id} className="rounded-sm border border-[var(--line)] bg-[rgba(10,15,26,0.72)] p-4">
              <div className="flex flex-wrap items-center gap-3">
                <span className="inline-flex h-7 w-7 items-center justify-center rounded-sm border border-[var(--line-bright)] bg-[var(--line-bright)] text-[14px] font-bold text-[var(--cool)]">
                  {index + 1}
                </span>
                <h3 className="text-[16px] font-semibold text-[var(--ink)]">
                  {step.title}
        ...
        <div className="dashboard-panel rounded-sm p-4 sm:p-5">
          ...
          <div className="mt-4 space-y-3">
            {resiliencePatterns.map((pattern) => (
              <p
                key={pattern}
                className="rounded-sm border border-[var(--line)] bg-[rgba(10,15,26,0.72)] px-4 py-3 text-[14px] leading-5 text-[var(--muted)]"
              >
                {pattern}
              </p>
            ))}
...
function InternalApiCard({ entry }: { entry: InternalApiDescriptor }) {
  return (
    <article className="rounded-sm border border-[var(--line)] bg-[rgba(10,15,26,0.72)] p-4">
      <div className="flex flex-wrap items-center gap-3">
        <code className="rounded-sm border border-[var(--line-bright)] bg-[var(--bg-raised)] px-2.5 py-1.5 text-[14px] text-[var(--ink)]">
          GET {entry.path}
        </code>
        <span className="rounded-sm border border-[var(--line-bright)] bg-[var(--line-bright)] px-2.5 py-1 text-[13px] font-bold uppercase tracking-[0.16em] text-[var(--cool)]">
          {entry.category}
        </span>
      </div>
  ...
  return (
    <article className="rounded-sm border border-[var(--line)] bg-[rgba(10,15,26,0.72)] p-4">
      <div className="flex flex-wrap items-center gap-3">
        ...
        {entry.optional ? (
          <span className="rounded-sm border border-[rgba(245,158,11,0.18)] bg-[rgba(245,158,11,0.12)] px-2.5 py-1 text-[13px] font-bold uppercase tracking-[0.16em] text-[#f59e0b]">
            Optional
          </span>
        ) : null}
      </div>
          ...
          <div className="mt-2 space-y-2">
            {entry.endpoints.map((endpoint) => (
              <code
                ...
                className="block overflow-x-auto rounded-sm border border-[var(--line)] bg-[var(--bg-raised)] px-3 py-2 text-[14px] text-[var(--ink)]"
              >
                {endpoint}
              </code>
            ))}
    ...
    <div className="space-y-5">
      <section className="rounded-sm border border-[#a855f7] bg-[rgba(168,85,247,0.08)] p-4 sm:p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <div className="text-[13px] font-bold uppercase tracking-[0.22em] text-[#c084fc]">
            ...
            <p className="mt-2 max-w-3xl text-[14px] leading-5 text-[var(--muted)]">
          ...
          <a
            ...
            className="inline-flex items-center gap-2 rounded-sm border border-[#a855f7] bg-[rgba(168,85,247,0.15)] px-4 py-2 text-[14px] font-semibold text-[#c084fc] transition-colors hover:bg-[rgba(168,85,247,0.25)]"
          >
    ...
    <div className="space-y-5">
      <section className="dashboard-panel rounded-sm p-4 sm:p-5">
        ...
        {statusPayload ? (
              ...
              {Object.entries(statusPayload.services).map(([serviceName, state]) => (
                ...
                >
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <span
                      className={`rounded-sm border px-2.5 py-1 text-[13px] font-bold uppercase tracking-[0.16em] ${stateBadgeClasses(
                        state,
                      )}`}
                    >
                      {state}
          ...
          <div className="mt-4 rounded-sm border border-[var(--line)] bg-[rgba(10,15,26,0.72)] px-4 py-3 text-[14px] leading-5 text-[var(--muted)]">
